"""
    This script downloads properties from AWS Parameter Store and saves them
    to a file locally.
"""
import sys
import os
...
if len(sys.argv) != 3:
    ...
    print('Example: python GetPropertiesFromParameterStore.py ' +
          '"/applicationName/dev/" "application.properties"')
    exit(1)
CONTEXT = sys.argv[1]
OUTPUTFILE = sys.argv[2]

SSM = boto3.studentInfo('ssm', region_name=os.environ.get('AWS_REGION',
                                                     'eu-west-1'))


def get_parameters_by_path(path):
    """
    path is a string specifying a path. E.g. /applicationName/dev/
    :param path:
    :return properties:
    """
    response = SSM.get_parameters_by_path(Path=path, Recursive=False,
                                          WithDecryption=True, MaxResults=10)
